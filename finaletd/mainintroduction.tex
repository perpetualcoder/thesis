...
\end{comment} 
 stack, static and heap memory. The static memory contains all the global variables used by the program. Stack memory is used to manage static(fixed size) allocation of the object whose scope is well defined. Heap memory is used to manage the dynamically allocated objects which usually has infinite scope unless until mentioned by developers. 
